...
let name = prompt("Welcome to 'Rock Paper Scissors' game,\nWhat is your name?");
// Capialize the prompt.
let capialize = function(arg) {
  return name[0].toUpperCase() + name.substr(1, arg.lenght).toLowerCase();
}
// Get the player's tool.
let p1 = prompt("Hey, " + capialize(name) + "!\nType r for Rock, p for Paper or s for Scissors.\nGood Luck!");
// Converting p1's prompt.
if (p1=="r") {
  p1 = "Rock";
} else if (p1=="p") {
  p1 = "Paper";
